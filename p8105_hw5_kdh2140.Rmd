...
The above code runs a simulation of the function previously created, sim_t_test, with various values of mu for 5000 times. It then binds the rows of the output and the unnests the data. 
...
The above set of plots represent the estimated value of mu for the sim_results dataset. The **first plot** includes all estimated values of mu, significant or not at alpha = 0.05. The **second plot** only includes the estimated values of mu that are significant at an alpha = 0.05. 

The sample average of mu across tests for which the null is rejected is not aprroximately equal to the true value of mu, as it only includes values that would be large enough to elicit a significance level larger than 0.05. The sample average of mu across tests is equal to the true value of mu in the first plot, which includes all values and allows for the a true calculation of mu derived from all samples. Note that when all samples are significant, such as mu = 4, 5, or 6, the average estimated value of mu in the second plot is equal to the true value. 
