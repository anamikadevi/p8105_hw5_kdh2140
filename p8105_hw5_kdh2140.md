...
is umported using the map function to read csv for each entry. The data
...
week and observation columns. The first few observations are displayed
for reference.
...
sim\_t\_test, with various values of mu, 5000 times over. It then binds
the rows of the output and unnests the data.
...
The above set of plots represent the average estimated value of mu by
true mu for the sim\_results dataset. The **first plot** includes all
estimated values of mu, significant or not at alpha = 0.05. The **second
plot** only includes the estimated values of mu that are significant at
an alpha = 0.05.
